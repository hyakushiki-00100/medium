...
- **翻訳臭の具体的なチェックポイント**（kissako-ja.md の「カテゴリー」指摘（2026-08-08）から追加）:
  ...
  - **英語の時間表現・二重否定をそのまま置き換えない。**（kufu-ja.md「カンフーが午後ひとつで身につく小技
    だなどとは思っていません」の指摘・2026-09-07）。EN "Nobody who has actually trained thinks kung fu
    is a trick you learn in an afternoon" を、① "in an afternoon" → 「午後ひとつで」のような**非慣用的な
    時間表現**（日本語では「一朝一夕で」「一日や二日で」等の慣用句を使う）、② "Nobody thinks X" の否定を
    「〜だなどとは思っていません」のような**伝聞・断定・否定を多重に重ねた構文**、のどちらの形であれ
    そのまま持ち込むと不自然になる。可能な限り**肯定形の知識文**（「〜だと知っています」「〜だと分かって
    います」）に組み替える。
...
> **オーナーからの再指摘（2026-09-05）**: note 版の日本語がまだ不自然（翻訳調）との指摘。上記チェックリストは
> 存在するだけでは不十分で、**執筆直後に必ず全文を音読確認**し、EN の構文・語順を頭の中で置き換えただけの
> 箇所（否定→肯定の転換文、体言止めの断片、EN のリズムをそのまま持ち込んだ接続）を洗い出してから確定させる。
> 「fact-checker のチェック項目に入っていないから素通りする」ことがないよう、**執筆者自身の自己レビューとして
> 独立に実施する**（fact-checker は史実性が主眼で、日本語の自然さそのものは主目的ではないため）。
...
1. **執筆(英語＋日本語)** — 英語=Medium(`articles/medium/<slug>-en.md`)、日本語=note(`articles/note/<slug>-ja.md`)。
   ...
   **タイトルの長さに上限を設ける**（2026-08-17〜、2026-08-27に基準を再引き締め）。mukanjo→bashoan→suikogyu の3記事で、
   ...
   一度 JA 70〜90字・EN 24〜32語という目安を設けたが、tokusantakuhatsu(EN 30語)でオーナーから再度
   「タイトルが長い。内容をただ説明している」と指摘された（2026-08-27）。**全記事のタイトル語数を
   実測して並べたところ、7〜21語のタイトルは軒並み「セットアップ→ひとひねり」の2ビートで歯切れよく
   読めるのに対し、24語を超えると (a) 出来事を3つ以上詰め込む、(b) 末尾を `— Which [誰か] called it X`
   のような従属節で締めて記事の結論・解釈をタイトル自身が説明してしまう、のどちらかが起きて
   冗長になる**、という傾向が判明した（tokusantakuhatsu 30語・muinoshinnin 34語・bashoan「婆子焼庵」36語・
   suikogyu 42語が実例）。
   - **目安を EN 18〜24語・JA 55〜75字に引き締める**（旧 24-32語/70-90字は「内容を全部説明できてしまう」
     余地を残しすぎていた）。
   - **ビートは最大2つ**（具体的な行動・場面 → 逆説やひとひねり）。3つ目のビートを足したくなったら、
     それは本文の冒頭段落に回す。
   - **タイトルは「事実・具体的な行動・逆説」で終わらせ、「それが何を意味するか／誰にどう評されたか」の
     説明はタイトルに書かない。** 意味づけをタイトル自身が語ると、記事を読む前に結論を渡してしまいフックが
     死ぬ（tokusantakuhatsu の初稿 `— Which Someone Called the Real Failure` が典型例。「誰か」という
   ...
   **暫定運用（2026-08-27〜、オーナー指示）**: しばらく**抽象的な洞察型をデフォルト**として新規記事の
   タイトルに採用し、10記事たまった時点でオーナーが振り返り分析を行う。新規セッションは、次のリストに
   スラッグと日付を追記していくこと（1記事目=tokusantakuhatsu）。**リストが10件に達したら、その回の
   作業報告の冒頭で「抽象的な洞察型タイトルが10記事に達しました。振り返りのタイミングです」と
   オーナーに明示的にリマインドすること**（記事を書くたび逐一確認は不要。10件到達時のみでよい）。
   リマインド後は、オーナーの指示があるまでこのデフォルト運用を維持してよい。
   ...
   **洞察型タイトル使用実績**:
   ...
   5. senpatsu（2026-09-04）
   6. chudo（2026-09-04、仏教語シリーズ）
   7. kasui（2026-09-05）
   8. kohin（2026-09-05）
   9. aisatsu（2026-09-05）
   10. kufu（2026-09-05）→ **10記事到達。振り返り実施済み(2026-09-05)。**

   > **オーナー決定（2026-09-05）**: 洞察型が10記事に到達したのを機に、場面描写型のタイトル案を
   > 提示したところ、**今後の新規記事10本は場面描写型で作成し、その後あらためて検討する**方針となった。
   > 次の新規記事（この決定より後に作成する最初の1本）から、洞察型ではなく場面描写型（EN 18〜24語 /
   > JA 55〜75字、2ビートまで、事実・逆説で終える。上記「場面描写型」節の基準に従う）をデフォルトにする。
   > 下記に使用実績を記録し、10本に達したら同様に振り返りを提案すること。

   **場面描写型タイトル使用実績（2026-09-05〜）**:
   1. seikousui（2026-09-07）
