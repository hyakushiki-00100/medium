...
5. **スレッドは「Mediumへの誘引」が目的であり、記事を出し尽くしてはいけない。作り終えたら必ず
   「CTAが読者に約束している中身は、記事にしかない実質的な情報か？」を自問する。** ここは一度
   ルール化(旧項目7)しておきながら、utekisei/houu/ukiseikou/aun の4本とも実践パートの問いや
   洞察を全文そのまま出し、CTAの中身が出典トリビアだけになっていた実例あり(2026-08-21、オーナー
   指摘で発覚・事後修正)。**ルールを書いただけでは守られない。スレッド完成のたびに毎回、
   この項目だけは目視で再確認すること。**
   - 問い・手順のリストは半分以下だけ出し、残りをCTAで名指しする。
   - 記事の主要な洞察ビートが1つ、意図的に丸ごとスレッドから抜けているか確認する
     （抜けているものが無い＝出し尽くしのサイン）。
   ```bash
   # CTAが出典トリビア(related phrase等)だけを売っていないか目視確認
   tail -6 social/x-thread-<slug>.md
   ```

### 推奨（クリック率・プラットフォームネイティブ感に直結）

6. **各ツイートは単独で意味が通る形で始める。** 矢印(→)始まり、番号リストのツイート跨ぎ(1)2)は同じ
